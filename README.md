...
| 510 Ω resistor (anything ~470 Ω–1 kΩ works) | resistor kit |
...
clock's internal antenna, and trigger a manual sync. A clean sync takes 2–5
minutes. Coupling is strongly directional — if nothing happens, rotate the
rod and vary the distance (2–15 cm) before suspecting anything else. Once
...
Metal does — keep sheet metal and foil away from the antenna.
...
frames — the second one must read exactly minute + 1. A frozen fake time is
...
1. **Full-off carrier dips beat the 15 % spec at close range.** The real
   ...
   (5 % PWM duty ≈ 15 % fundamental). The clock detected seconds but never
   accepted a frame — at centimeter distance the receiver's AGC flattens a
   partial dip. Switching to carrier-fully-off (`DUTY_REDUCED = 0`) fixed it
   immediately. Don't "restore spec behavior" without retesting.
...
3. **A blinking reception icon on the clock is a signal-presence detector,
   not a decode indicator.** Pulling the transmitter's power and watching the
   icon stop blinking is a great causality test.
...
hundreds of km — the receiver's AGC locks onto the dominant signal. If in
...
exactly one hour off — the original problem this project solves.

**Is the broken ferrite rod a problem for transmission?**
No — maybe a factor 1.5–3 in field strength, which 2 cm less distance more
than compensates. Glued halves lose almost nothing.
...
side of the wire — enamel isolates even against probe tips. Typically only
1–3 mm around solder joints are affected.
